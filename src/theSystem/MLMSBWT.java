...
public class MLMSBWT {
	...
	public MLMSBWT(ArrayList<Persona> inputList, int i) {
	...
	public static void processSLMS(ArrayList<Persona> inputList, int amountOfClerks){
		...
		MonitorMLMSBWT getFastestLine = new MonitorMLMSBWT(lineQ, pQ);
		...
		while(!inputList.isEmpty() || !lineQ.allLinesEmpty() || !pQ.allPostsEmpty()){
			...
			while((!inputList.isEmpty()) && t>=inputList.get(0).getArrivalTime()) {
				getFastestLine = new MonitorMLMSBWT(lineQ, pQ);
				lineQ.getFila(getFastestLine.theMonitorMethod(lineQ, pQ)).enqueue(inputList.get(0));
				inputList.remove(0);
			}
